...
void handleData() {
  ...
  //If up/down is pressed, display is blinking so don't store that as a temperature value. (skip until tempvalid time occurs)
...
void forceSetHeater(bool state) {
  if (state != (heater_red_sts || heater_green_sts)) {
    if (unlockDevice()) {
      virtualBTN = HTR;
      heater_cmd = state;
